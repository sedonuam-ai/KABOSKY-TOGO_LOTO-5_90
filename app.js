...
// Donne le jour actuel en français, pour repérer "le jeu du jour"
...
function renderHome() {
  ...
  const gamesToday = GAMES.filter(g => g.day === today);

  container.innerHTML = `
    <div class="step-card" style="cursor:default; align-items:flex-start;">
      <div class="step-number">1</div>
      <div class="step-text" style="width:100%;">
        <div class="step-title">Choisir le jeu du jour</div>
        <div class="step-sub">Aujourd'hui : ${today}</div>
        <div id="today-games" style="margin-top:8px;"></div>
      </div>
    </div>
  ...
  // ----- Étape 1 : le(s) jeu(x) du jour -----
  const todayContainer = document.getElementById('today-games');
  if (gamesToday.length === 0) {
    todayContainer.innerHTML = `<div class="empty-msg">Pas de tirage le ${today}.</div>`;
  } else {
    gamesToday.forEach(game => todayContainer.appendChild(buildGameCard(game)));
  }
